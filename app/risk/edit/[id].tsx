        ...
        <View style={[styles.mainLayout, isDesktop && styles.mainLayoutDesktop]}>
          ...
          <View style={{ flex: 1 }}>
                ...
                {step === 0 && (
                  <View style={styles.formFields}>
                    ...
                    <FormField label="Fonte de Risco" colors={colors}>
                      <View style={styles.optionsGrid}>
                        {FONTES_DE_RISCO.map(fonte => (
                          <TouchableOpacity key={fonte} style={[styles.optionChip, { borderColor: risk.fonteDeRisco === fonte ? colors.primary : colors.border }, risk.fonteDeRisco === fonte && { backgroundColor: colors.primary + '12' }]} onPress={() => updateField('fonteDeRisco', fonte)} activeOpacity={0.7}>
                    ...
                    <FormField label="Descrição da Fonte" colors={colors}>
                      <TextInput style={[styles.textArea, { color: colors.foreground, backgroundColor: colors.surface, borderColor: colors.border }]} value={risk.descricaoFonte} onChangeText={v => updateField('descricaoFonte', v)} multiline numberOfLines={3} />
                    </FormField>
                    <FormField label="Ameaça" colors={colors}>
                      <TextInput style={[styles.input, { color: colors.foreground, backgroundColor: colors.surface, borderColor: colors.border }]} value={risk.ameaca} onChangeText={v => updateField('ameaca', v)} returnKeyType="done" />
                    </FormField>
                  </View>
                )}
                ...
                {step === 1 && (
                  <View style={styles.formFields}>
                    ...
                    {risk.ameaca && risk.consequencia ? (
                      <TouchableOpacity style={[styles.generateBtn, { backgroundColor: colors.primary + '12', borderColor: colors.primary }]} onPress={generateForma3} activeOpacity={0.7}>
                        <Text style={{ color: colors.primary, fontWeight: '600', fontSize: 14 }}>Gerar Forma 3 automaticamente</Text>
                      </TouchableOpacity>
                    ) : null}
                    <FormField label="Consequência" colors={colors}>
                      <TextInput style={[styles.textArea, { color: colors.foreground, backgroundColor: colors.surface, borderColor: colors.border }]} value={risk.consequencia} onChangeText={v => updateField('consequencia', v)} multiline numberOfLines={3} />
                    </FormField>
                    <FormField label="Descrição do Risco (Forma 3)" colors={colors}>
                      <TextInput style={[styles.textArea, { color: colors.foreground, backgroundColor: colors.surface, borderColor: colors.border, minHeight: 100 }]} value={risk.descricaoRisco} onChangeText={v => updateField('descricaoRisco', v)} multiline numberOfLines={4} />
                    </FormField>
                  </View>
                )}
                    ...
                    <FormField label="Risco Estratégico?" colors={colors}>
                      <View style={styles.toggleRow}>
                        {(['SIM', 'NÃO'] as const).map(opt => (
                          <TouchableOpacity key={opt} style={[styles.toggleBtn, { borderColor: risk.estrategico === opt ? colors.primary : colors.border }, risk.estrategico === opt && { backgroundColor: colors.primary + '12' }]} onPress={() => updateField('estrategico', opt)} activeOpacity={0.7}>
                    ...
                    <FormField label="Tipo de Risco" colors={colors}>
                      <View style={styles.optionsGrid}>
                        {TIPOS_DE_RISCO.map(tipo => (
                          <TouchableOpacity key={tipo} style={[styles.optionChip, { borderColor: risk.tipoRisco === tipo ? colors.primary : colors.border }, risk.tipoRisco === tipo && { backgroundColor: colors.primary + '12' }]} onPress={() => updateField('tipoRisco', tipo)} activeOpacity={0.7}>
                    ...
                    <FormField label="Estratégia de Tratamento (MATE)" colors={colors}>
                      <View style={styles.optionsGrid}>
                        {ESTRATEGIAS_TRATAMENTO.map(est => (
                          <TouchableOpacity key={est} style={[styles.optionChip, { borderColor: risk.tratamento === est ? colors.primary : colors.border }, risk.tratamento === est && { backgroundColor: colors.primary + '12' }]} onPress={() => updateField('tratamento', est)} activeOpacity={0.7}>
                            <Text style={[styles.optionText, { color: risk.tratamento === est ? colors.primary : colors.foreground }]}>{est}</Text>
                          </TouchableOpacity>
                        ))}
                      </View>
                    </FormField>
                    <FormField label="Controles" colors={colors}>
                      <TextInput style={[styles.textArea, { color: colors.foreground, backgroundColor: colors.surface, borderColor: colors.border }]} value={risk.controles} onChangeText={v => updateField('controles', v)} multiline numberOfLines={3} />
                    </FormField>
                    <View style={[styles.formRow, isDesktop && styles.formRowDesktop]}>
                      <View style={{ flex: 1 }}>
                        <FormField label="Responsável" colors={colors}>
                          <TextInput style={[styles.input, { color: colors.foreground, backgroundColor: colors.surface, borderColor: colors.border }]} value={risk.responsavel} onChangeText={v => updateField('responsavel', v)} returnKeyType="done" />
                        </FormField>
                      </View>
                      <View style={{ flex: 1 }}>
                        <FormField label="Prazo" colors={colors}>
                          <TextInput style={[styles.input, { color: colors.foreground, backgroundColor: colors.surface, borderColor: colors.border }]} value={risk.prazo} onChangeText={v => updateField('prazo', v)} returnKeyType="done" />
                        </FormField>
                      </View>
                    </View>
                    <FormField label="KRI" colors={colors}>
                      <TextInput style={[styles.textArea, { color: colors.foreground, backgroundColor: colors.surface, borderColor: colors.border }]} value={risk.kri} onChangeText={v => updateField('kri', v)} multiline numberOfLines={3} />
                    </FormField>
                  </View>
                )}
                ...
                {step === 6 && (
                  <View style={styles.formFields}>
                    ...
                    <FormField label="Eficácia do Tratamento" colors={colors}>
                      <TextInput style={[styles.textArea, { color: colors.foreground, backgroundColor: colors.surface, borderColor: colors.border }]} value={risk.eficaciaTratamento} onChangeText={v => updateField('eficaciaTratamento', v)} multiline numberOfLines={4} />
                    </FormField>
                  </View>
                )}
              </View>
            ...
            <View style={[styles.bottomNav, { backgroundColor: colors.surface, borderTopColor: colors.border }, isDesktop && styles.bottomNavDesktop]}>
              {step > 0 ? (
                <TouchableOpacity style={[styles.navBtn, styles.navBtnSecondary, { borderColor: colors.border }]} onPress={() => setStep(s => s - 1)} activeOpacity={0.7}>
                  <Text style={{ color: colors.foreground, fontWeight: '600' }}>Anterior</Text>
                </TouchableOpacity>
              ) : (
                <TouchableOpacity style={[styles.navBtn, styles.navBtnSecondary, { borderColor: colors.border }]} onPress={() => router.back()} activeOpacity={0.7}>
                  <Text style={{ color: colors.error, fontWeight: '600' }}>Cancelar</Text>
                </TouchableOpacity>
              )}

              {step < STEPS.length - 1 ? (
                <TouchableOpacity style={[styles.navBtn, { backgroundColor: colors.primary }]} onPress={() => setStep(s => s + 1)} activeOpacity={0.7}>
                  <Text style={{ color: '#fff', fontWeight: '600' }}>Próximo</Text>
                </TouchableOpacity>
              ) : (
                <TouchableOpacity style={[styles.navBtn, { backgroundColor: '#38A169' }]} onPress={handleSave} activeOpacity={0.7}>
    ...
    <View style={styles.fieldGroup}>
      ...
      <View style={styles.sliderRow}>
        {[1, 2, 3, 4, 5].map(n => {
          ...
          return (
            <TouchableOpacity key={n} style={[styles.sliderBtn, { borderColor: isSelected ? levelColor : colors.border }, isSelected && { backgroundColor: levelColor + '15' }]} onPress={() => onChange(n)} activeOpacity={0.7}>
